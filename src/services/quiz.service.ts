...
export class QuizService {
    ...
    const quiz = await prisma.quiz.findUnique({
      ...
      include: {
        questions: {
          orderBy: { order: "asc" },
          include: {
            options: { orderBy: { order: "asc" }, select: { id: true, textEn: true, textFil: true, order: true } },
          },
        },
  ...
  async submitQuiz(userId: string, dto: SubmitQuizDto) {
    ...
    const attemptCount = await prisma.quizAttempt.count({ where: { userId, quizId: dto.quizId } });
    if (attemptCount >= quiz.maxAttempts) throw new Error("Maximum attempts reached");

    // Grade answers
    let score = 0;
    let maxScore = 0;
    const gradedAnswers: Array<{ questionId: string; selectedOption: string; isCorrect: boolean }> = [];

    for (const question of quiz.questions) {
      maxScore += question.points;
      const submitted = dto.answers.find((a) => a.questionId === question.id);
      if (!submitted) {
        gradedAnswers.push({ questionId: question.id, selectedOption: "", isCorrect: false });
        continue;
      }
      const option = question.options.find((o) => o.id === submitted.selectedOptionId);
      const isCorrect = option?.isCorrect ?? false;
      if (isCorrect) score += question.points;
      gradedAnswers.push({ questionId: question.id, selectedOption: submitted.selectedOptionId, isCorrect });
    }
    ...
    if (quiz.type === "PRE_TEST") {
      await prisma.moduleLockStatus.upsert({
        ...
        update: {
          ...
          isUnlocked: isPassed,
          lockReason: isPassed ? (quiz.module.requiresAllLessons ? "LESSONS_INCOMPLETE" : "UNLOCKED") : "AWAITING_PRETEST",
        },
        create: {
          userId,
    ...
    if (quiz.type === "POST_TEST" && isPassed) {
      await prisma.moduleLockStatus.upsert({
        where: { userId_moduleId: { userId, moduleId: quiz.moduleId } },
        update: { posttestPassed: true, isUnlocked: true, lockReason: "UNLOCKED", unlockedAt: new Date() },
        create: { userId, moduleId: quiz.moduleId, posttestPassed: true, isUnlocked: true, lockReason: "UNLOCKED", unlockedAt: new Date() },
      });
  ...
  async createQuiz(data: {
    moduleId: string; titleEn: string; titleFil: string;
    type: string; passingScore?: number; maxAttempts?: number;
    timeLimitMin?: number; blocksProgress?: boolean;
  }) {
    return prisma.quiz.create({ data: data as any });
  }

  async addQuestion(quizId: string, data: {
    textEn: string; textFil: string; order?: number; points?: number;
    options: Array<{ textEn: string; textFil: string; isCorrect: boolean; order?: number }>;
  }) {
