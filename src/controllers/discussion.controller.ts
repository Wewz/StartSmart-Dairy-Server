...
export const getThreads = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const data = await discussionService.getThreads(
...
export const getThread = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const thread = await discussionService.getThread(param(req.params.id));
...
export const deleteThread = async (
  ...
  try {
    await discussionService.deleteThread(
      param(req.params.id),
      req.user!.userId,
      req.user!.role,
    );
...
export const pinThread = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const thread = await discussionService.pinThread(
      param(req.params.id),
      req.body.isPinned,
    );
    return ok(res, thread);
